...
export default function AssignInput({
  members,
  setMembers,
  selectedMembers,
  setSelectedMembers,
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await api.get(`/api/project/membres/${projectId}`)
        setMembers(response.data.members)
        selectedMembers.length > 0
          ? setMembersToDisplay(() =>
              response.data.members.filter((member: User) => !selectedMembers.includes(member._id))
            )
          : setMembersToDisplay(response.data.members)
      } catch (error) {
        console.error(error)
      }
    }
  ...
  //       const conversations: User[] = response.data.members.map(
  //         (user: { _id: string; name: string; profilePicture: string }) => {
  //           return { id: user._id, name: user.name, profilePicture: user.profilePicture }
  //         }
  //       )
  //       if (members !== undefined) {
  //         const membersIds = members.map((member) => member.id)
  //         const selectedMembers = conversations.filter((user) => membersIds.includes(user.id))
  //         setMembers(selectedMembers)
  //         setRemainingUsers(conversations.filter((user) => !membersIds.includes(user.id)))
  //       }
  //       setUsers(conversations)
  //     } catch (error: unknown | AxiosError) {
  //       //handleError(error, logout)
  //       if (axios.isAxiosError(error)) {
  //         error.response && error.response.data.error.toLowerCase() === 'conversation not found'
  //         setUsers([])
  //       }
  //     }
  //   })()
